...
const mdbhostfromenv = process.env.IAM_MDB_PORT_27017_TCP_ADDR;
console.log("got mdbhost from environment: " + mdbhostfromenv);

const mdbhost = mdbhostfromenv || "127.0.0.1";
console.log("using mdbhost: ", mdbhost);
...
function CRUDImpl(tenant) {
    ...
    // if we have a tenant, the path needs to be considerd rather than the uri for database access
    this.processRequest = function(req, res, apiref, path) {
        ...
        if (req.method == "POST" && utils.startsWith(req.headers["content-type"], "multipart/form-data;")) {
            ...
            mdbclient.connect(databaseBaseUrl, {
                ...
                console.log(databaseLogPrefix + ".processRequest(): path (in case of multi tenancy): " + path);
                ...
                console.log(databaseLogPrefix + ".processRequest(): uri for mdb request: " + uri);
                ...
                var mdbrequest = new MDBRequest(tenant ? path : uri);
                console.log(databaseLogPrefix + ".processRequest(): mdbrequest: ", mdbrequest);
    ...
    function handleMultipartRequest(uri, req, res) {
        ...
        multipart.handleMultipartRequest(req, res, "./www/", (tenant ? tenant.id + "/" : "") + "content/", function ondone(formdata) {
            if (formdata && formdata.error) {
                console.log(databaseLogPrefix + ".handleMultipartRequest(): got error: " + formdata.error);
                res.writeHead(500);
                res.write(formdata.error);
                res.end();
            }
            else {
